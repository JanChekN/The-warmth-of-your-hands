...
label pr_scene_1:
    ...
    if gg_say == 1:
        ...
        ray 'А тебя как?'
        gg 'Меня зовут [name]'
        ray 'Приятно познакомится, [name]'


    '[love_points] - очки симпатии'
    

    jump pr_scene_2

label pr_scene_2:
    'Спустя несколько минут. [ray_name], согреваясь капучино, медленно потягивает напиток, опуская взгляд в чашку. 
    [name] продолжает работать, но иногда незаметно посматривает в её сторону.'
    '[ray_name] вдруг ставит чашку на столик и смотрит прямо на тебя.'
...
label pr_scene_3:
    if ray_name == 'Рэй':
        'Однажды, убирая со стола после ухода [ray_name], [name] замечает на салфетке небрежные строчки. 
        Они написаны чёрной гелевой ручкой, немного размытой влагой.'
    elif ray_name == 'Рэй' and partner_gender == 'f':
        'Однажды, убирая со стола после ухода [ray_name]и, [name] замечает на салфетке небрежные строчки. 
        Они написаны чёрной гелевой ручкой, немного размытой влагой.'
    else:
        'Контент недоступен в этой версии'

    'Записка: Люди возвращаются в места, которые делают их счастливыми.'

    menu:
        'Оставить записку':
            $ gg_say = 1

        'Выбросить, не придавая значения':
            $ gg_say = 2

        'Написать ответ и оставить на столе':
            $ gg_say = 3

    if gg_say == 3:
        $ love_points += 2
        'Иногда мы возвращаемся не к месту, а к людям'

        'Через день...'
        ray 'Интересная мысль.'
